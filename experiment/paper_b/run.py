...
def assert_formal_run_provenance(protocol: dict[str, Any], smoke: bool) -> dict[str, Any]:
    ...
    if not smoke and policy.get("require_clean_git_for_formal", True) and snapshot.get("git_dirty"):
        ...
        raise RuntimeError(
            ...
            "Commit the experiment code first, or explicitly disable the policy in paper_b.yaml.\n"
            f"Dirty paths:\n{preview}"
        )
    return snapshot
...
def assert_frozen_pilot_selection(
    ...
    if smoke:
        return None
    path = PAPER_B_DIR / "selection" / "pilot_decision.json"
    if not path.exists():
        raise RuntimeError(
            "Formal test runs require a frozen validation-only pilot decision. "
            "Run `python -m experiment.paper_b.pilot freeze`, commit it, then retry."
        )
...
def train_one(
    ...
    if not smoke:
        ...
        if stored_audit.get("fatal") or not stored_audit.get("content_hashes_checked"):
            raise RuntimeError(
                "Re-run `python -m experiment.paper_b.run audit --dataset "
    ...
    selection_path = PAPER_B_DIR / "selection" / "pilot_decision.json"
