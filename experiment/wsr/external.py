...
try:
    from .common import REPRODUCIBILITY_DIR, PROJECT_DIR, class_names, load_yaml, resolve_path
...
def detr_overlays(
    ...
    overlay_dir = workdir / "configs" / "wsr"
...
def materialize(protocol: dict[str, Any], dataset: str, repos_root: Path, device: str) -> Path:
    ...
    if not coco.exists():
        raise FileNotFoundError(
            f"COCO conversion is missing. Run: python -m experiment.wsr.run prepare --dataset {dataset} --coco"
        )
    ...
    adapter = REPRODUCIBILITY_DIR / "external_ultralytics.py"
    rfdetr_adapter = REPRODUCIBILITY_DIR / "external_rfdetr.py"
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=REPRODUCIBILITY_DIR / "protocol.yaml")
