...
```powershell
python -m experiment.wsr.single_model_inference `
  --candidate path/to/inspected.jpg `
  --reference path/to/aligned_golden.jpg `
  --policy experiment/wsr/generated/evidence/frozen_structural_safe_policy.json `
...
`python -m pytest experiment/wsr/tests -q` completed with all 43 tests
passing. An end-to-end deployment smoke test is recorded at
