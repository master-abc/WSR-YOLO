...
try:
    ...
    from .common import REPRODUCIBILITY_DIR, atomic_json_dump, environment_snapshot, file_hashes, sha256_file
...
def evaluate_gate(protocol: dict[str, Any]) -> dict[str, Any]:
    ...
    decision = {
        ...
        "source_files_sha256": file_hashes(
            [Path(__file__), REPRODUCIBILITY_DIR / "ablation.py", REPRODUCIBILITY_DIR / "mechanism_diagnostics.py", REPRODUCIBILITY_DIR / "benchmark.py"]
        ),
...
def freeze_decision(protocol: dict[str, Any]) -> Path:
    ...
    target = REPRODUCIBILITY_DIR / "selection" / "pilot_decision.json"
    if target.exists():
